...
| H — store | **PASS** | 2026-09-14: `src/store.h/.cpp` over the extended `storewindow` model; client 0xFA/0xFB/0xFC/0xFD/0xFE/0xE8/0xE9/0xEF, server 0xFB/0xFC/0xFD/0xFE/0xE0/0xEA/0xDF/0xF2; headless 15.25 client: categories, front page with banners, offers, purchase into the store inbox, outfit purchase then shown as owned, search, coin transfer to another account, paged history, balances right after every step, zero decode errors, 10/10 tests. Coins on the account with write-through and `store_history`. |
