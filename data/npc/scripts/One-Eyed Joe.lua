...
local function creatureSayCallback(cid, type, msg)
	...
	if msgcontains(msg, "mission") and player:getStorageValue(Storage.Quest.U10_70.TheCursedCrystal.Oneeyedjoe) < 0 and npcHandler.topic[playerId] < 1 then
		npcHandler:say({
	...
	elseif msgcontains(msg, "yes") and player:getStorageValue(Storage.Quest.U10_70.TheCursedCrystal.Oneeyedjoe) > 0 and player:getStorageValue(Storage.Quest.U10_70.TheCursedCrystal.Oneeyedjoe) < 3 then
		npcHandler:say("Hmm. No, I don't think so. I still feel this strange prickling in my toes.", cid)
		npcHandler.topic[playerId] = 0
	elseif msgcontains(msg, "no") and player:getStorageValue(Storage.Quest.U10_70.TheCursedCrystal.Oneeyedjoe) > 0 and player:getStorageValue(Storage.Quest.U10_70.TheCursedCrystal.Oneeyedjoe) < 3 then
	...
	elseif msgcontains(msg, "crystals") then
		...
		npcHandler.topic[playerId] = 0
	elseif msgcontains(msg, "cursed") and player:getStorageValue(Storage.Quest.U10_70.TheCursedCrystal.Oneeyedjoe) < 0 and npcHandler.topic[playerId] < 1 then
		npcHandler:say({
			"As for myself I haven't been down there. But I heard some disturbing rumours. In these caves are wonderful crystal formations. Some more poetically inclined fellows call them the crystal gardens. ...",
			"At first glance it seems to be a beautiful - and precious - surrounding. But in truth, deep down in these caverns exists an old evil. Want to hear more?",
		}, cid)
	...
	elseif player:getStorageValue(Storage.Quest.U10_70.TheCursedCrystal.Oneeyedjoe) >= 0 and player:getStorageValue(Storage.Quest.U10_70.TheCursedCrystal.Oneeyedjoe) < 3 then
		npcHandler:say("Ah, the brave adventurer who sought to destroy the evil crystal down there. Have you been successful?", cid)
		npcHandler.topic[playerId] = 0
	end
