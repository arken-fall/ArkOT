...
local function creatureSayCallback(cid, type, msg)
	...
	if msgcontains(msg, "research") and player:getStorageValue(Storage.Quest.U8_7.JackFutureQuest.QuestLine) == 11 then
	...
	elseif msgcontains(msg, "mission") then
		...
		elseif (player:getStorageValue(Storage.Quest.U8_54.SeaOfLight.Questline) == 10) and (player:getStorageValue(Storage.Quest.U8_7.JackFutureQuest.QuestLine) < 1) then
			npcHandler:say("After the debacle with the crystal, I started focussing on other things. There are also some {tasks} that still need to be done. If you can spare the time to continue helping me, it shall not be to your disadvantage. So are you in for another mission?", cid)
		...
		elseif player:getStorageValue(Storage.Quest.U8_7.JackFutureQuest.QuestLine) == 4 then
			npcHandler:say("You're back from {Jack}! Mh, by the looks of your face I doubt our little redecoration project yielded any success. But I had an even better idea while you were gone - ready to give it another try?", cid)
		...
		elseif player:getStorageValue(Storage.Quest.U8_7.JackFutureQuest.QuestLine) == 8 then
			npcHandler:say("Did you find out what hobby {Jack} has? Did you separate him from this activity? Only if he has a free mind, he can truly get back to his former self! Now all you need to do is talk to him again!", cid)
			npcHandler.topic[playerId] = 0
	...
	elseif msgcontains(msg, "yes") then
		if npcHandler.topic[playerId] == 1 and player:getStorageValue(Storage.Quest.U8_54.SeaOfLight.Questline) < 1 then
		...
		elseif npcHandler.topic[playerId] == 3 then
			npcHandler:say("One remaining mirror crystal is in the hands of a cid called the collector which collects all kinds of crystals. The only way to get access to its lair is to donate a very rare crystal to a secret well. I need you to get one, will you help me?", cid)
		...
		elseif npcHandler.topic[playerId] == 30 then
			npcHandler:say("Yes? And he didn't remember anything? Not even me? That's not good. Then we will have to do everything ourselves. Are you ready to continue?", cid)
	...
	if msgcontains(msg, "machine") and player:getStorageValue(Storage.Quest.U9_4.LiquidBlackQuest.Visitor) == 3 then
		npcHandler:say({
	...
	elseif msgcontains(msg, "yes") and npcHandler.topic[playerId] == 21 and player:getStorageValue(Storage.Quest.U9_4.LiquidBlackQuest.Visitor) == 3 then
	...
	elseif msgcontains(msg, "yes") and npcHandler.topic[playerId] == 22 and player:getStorageValue(Storage.Quest.U9_4.LiquidBlackQuest.Visitor) == 3 then
		if player:getStorageValue(Storage.Quest.U9_4.LiquidBlackQuest.Visitor) == 3 then
			npcHandler:say({
	...
	elseif msgcontains(msg, "yes") and npcHandler.topic[playerId] == 23 and player:getStorageValue(Storage.Quest.U9_4.LiquidBlackQuest.Visitor) == 3 then
		if player:getStorageValue(Storage.Quest.U9_4.LiquidBlackQuest.Visitor) == 3 then
			npcHandler:say("Then off you go! Im sorry that I cannot offer you any further help but Im sure you will find support along your way. And - be careful. The sea can appear pitch black down there.", cid)
	...
	elseif msgcontains(msg, "yes") then
		if npcHandler.topic[playerId] == 26 then
			npcHandler:say({
				"Well, if you really want to delve into this - I could use some help. So you have found my machine on that island? And you found the notes with the coordinates? Then you can find the entrance! ...",
				"Just look for a large staircase with sprawling steps. There is an unpassable stream there that will prevent you from venturing further on. But fear not, you can indeed travel down there - with these small enhancements I created. ...",
