...
local function creatureSayCallback(cid, type, msg)
	...
	if table.contains({ "mission", "hunt", "passage" }, msg:lower()) then
		if msgcontains(msg, "passage") then
			if player:getStorageValue(Storage.Quest.U8_2.TheHuntForTheSeaSerpent.FishForASerpent) < 5 then
				npcHandler:say("Hold your horses! First we need to get more {bait} fo' the sea serpent. Bring me the fish I requested and we can set sails immediately.", cid)
				return true
			end
		end
		...
		elseif player:getStorageValue(Storage.Quest.U8_2.TheHuntForTheSeaSerpent.FishForASerpent) < 5 then
			npcHandler:say("You got any {baits} for me?", cid)
		elseif player:getStorageValue(Storage.Quest.U8_2.TheHuntForTheSeaSerpent.FishForASerpent) >= 5 then
	...
	elseif msgcontains(msg, "yes") then
		...
		elseif npcHandler.topic[playerId] == 2 then
			npcHandler:say("Let's go fo' a {hunt} and bring the beast down!", cid) --test
	...
	elseif msgcontains(msg, "bait") then
		...
		elseif player:getStorageValue(Storage.Quest.U8_2.TheHuntForTheSeaSerpent.FishForASerpent) >= 5 then
			npcHandler:say("The bait is ready, tell me if ya're ready to start the hunt.", cid)
		end
	end
	return true
end
