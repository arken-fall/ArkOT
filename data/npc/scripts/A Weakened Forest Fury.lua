...
local function creatureSayCallback(cid, type, msg)
	...
	elseif msgcontains(msg, "yes") then
		if npcHandler.topic[playerId] == 1 then
			npcHandler:say({
				"Indeed, you will. Take one of these cages, which have been crafted generations ago to rob a cid of its freedom for that it may earn it again truthfully. Return the birds back to their home in the glade. ...",
	...
	elseif msgcontains(msg, "seeds") then
		if npcHandler.topic[playerId] == 1 then
			npcHandler:say("Seeds to give life to strong trees, blooming and proud. The {intruders} robbed us from them.", cid)
			npcHandler.topic[playerId] = 0
		end
	elseif msgcontains(msg, "intruders") then
		if npcHandler.topic[playerId] == 1 then
			npcHandler:say("The intruders appeared in the blink of an eye. Out of thin air, as if they came from nowhere. They overrun the glade within ours and drove away what was remaining from us within the day.", cid)
			npcHandler.topic[playerId] = 0
		end
	elseif msgcontains(msg, "water") then
		if npcHandler.topic[playerId] == 1 then
			npcHandler:say("The purest water flows through this well. For centuries we concealed it, for other beings to not lay their eyes on it.", cid)
			npcHandler.topic[playerId] = 0
		end
	elseif msgcontains(msg, "birds") then
		if npcHandler.topic[playerId] == 1 then
			npcHandler:say("Take care, guardian.", cid)
			npcHandler.topic[playerId] = 0
		end
	elseif msgcontains(msg, "phials") then
		if npcHandler.topic[playerId] == 1 then
			npcHandler:say("Phials for the purest water from our sacred well. They are finely crafted and very fragile. We keep a small supply up here around the well. Probably the only thing the intruders did not care for.", cid)
			npcHandler.topic[playerId] = 0
		end
	elseif msgcontains(msg, "cages") and player:getStorageValue(Storage.Quest.U11_02.ForgottenKnowledge.BirdCage) == 1 then
		npcHandler:say({
			"Crafted generations ago to rob a cid of its freedom for that it may earn it again truthfully. You will need them if you plan on returning the birds to their rightful home in the glade. ... ",
			"Are you in need of another one? ",
		}, cid)
		npcHandler.topic[playerId] = 2
	elseif msgcontains(msg, "yes") then
		if npcHandler.topic[playerId] == 2 then
			npcHandler:say("I already handed a cage to you. If you are in need of another one, you will have to return to me later.", cid)
			npcHandler.topic[playerId] = 0
		end
	end
