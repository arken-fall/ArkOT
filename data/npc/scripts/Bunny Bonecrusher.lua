...
local function creatureSayCallback(cid, type, msg)
	...
	if msgcontains(msg, "mission") then
		...
		elseif player:getStorageValue(Storage.Quest.U8_1.TowerDefenceQuest.Questline) == 4 then
			npcHandler:say("Alright, you delivered the parcel. So what is the password Thanita told you?", cid)
			npcHandler.topic[playerId] = 2
		end
	elseif npcHandler.topic[playerId] == 1 and msgcontains(msg, "yes") then
		npcHandler:say("I am not sure if I should be glad now or not but anyway ... you will get a password so I will know if you just threw it away or actually delivered it. Here is the parcel. See you ....or not.", cid)
		player:addItem(140, 1)
		player:setStorageValue(Storage.Quest.U8_1.TowerDefenceQuest.Questline, 1)
		npcHandler.topic[playerId] = 0
	elseif npcHandler.topic[playerId] == 2 and msgcontains(msg, "password*") then
		npcHandler:say("That's right. Here is your reward some elementary arrows. You did pretty well on your mission!", cid)
