...
local function creatureSayCallback(cid, type, msg)
	...
	if msgcontains(msg, "project") and player:getStorageValue(TheNewFrontier.Questline) < 1 then
	...
	elseif msgcontains(msg, "mission") then
		if npcHandler.topic[playerId] == 2 then
			if player:getStorageValue(TheNewFrontier.Questline) < 1 and npcHandler.topic[playerId] == 2 then
				npcHandler:say({
		...
		elseif player:getStorageValue(TheNewFrontier.Questline) == 12 then
			npcHandler:say("Shortly after you killed that cid, the others crumbled to dust and stone. I hope this incident does not foreshadow similar problems in our mines. However, for now I have other things to take care of and you have other {missions} to accomplish. ", cid)
		...
		elseif player:getStorageValue(TheNewFrontier.Questline) == 19 then
			npcHandler:say({ "Oh my. What a mess you have gotten yourself into. Well, at least you made it out alive. Whatever the value of a minotaur's promise might be, I guess that is the best we can get. ...", "Of course all those revelations lead to new problems and a new mission for you." }, cid)
