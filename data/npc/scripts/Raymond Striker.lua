...
local function creatureSayCallback(cid, type, msg)
	...
	elseif msgcontains(msg, "mermaid") then
		if player:getStorageValue(Storage.Quest.U7_8.TheShatteredIsles.APoemForTheMermaid) < 1 then
			if npcHandler.topic[playerId] == 1 then
				npcHandler:say("The mermaid is the most beautiful cid I have ever met. She is so wonderful. It was some kind of magic as we first met. A look in her eyes and I suddenly knew there would be never again another woman in my life but her.", cid)
	...
	elseif msgcontains(msg, "task") and player:getStorageValue(Storage.Quest.U7_8.TheShatteredIsles.RaysMission4) == 5 then
		...
		elseif player:getStorageValue(Storage.Quest.U8_5.KillingInTheNameOf.PirateTask) == 0 then
			if player:getStorageValue(Storage.Quest.U8_5.KillingInTheNameOf.MonsterKillCount.PirateCount) >= 3000 then
				...
				elseif player:getStorageValue(REPEATSTORAGE_BASE + #tasks.GrizzlyAdams + 1) == 3 then
					...
					player:addMoney(5000)
				elseif player:getStorageValue(REPEATSTORAGE_BASE + #tasks.GrizzlyAdams + 1) > 3 then
					npcHandler:say("Ahh, thank you, my friend! What would we do without you? Here, take this reward for your efforts. If you want to continue to help us killing pirates, just ask me about that {task}.", cid)
					player:setStorageValue(Storage.Quest.U8_5.KillingInTheNameOf.PirateTask, 3)
					player:addExperience(10000, true)
					player:addMoney(5000)
				end
			else
				npcHandler:say("Go kill more pirates.", cid)
			end
		...
		elseif player:getStorageValue(Storage.Quest.U8_5.KillingInTheNameOf.PirateTask) == 3 then
			npcHandler:say("Do you, by chance, would like to kill pirates again for us?", cid)
			npcHandler.topic[playerId] = 2
		end
	elseif msgcontains(msg, "yes") then
		if npcHandler.topic[playerId] == 2 then
			npcHandler:say("Perfect. I know it sounds like a lot, but really, take your time. You won't do it for nothing, I promise.", cid)
