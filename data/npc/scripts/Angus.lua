...
local function creatureSayCallback(cid, type, msg)
	...
	elseif msgcontains(msg, "farmine") and player:getStorageValue(TheNewFrontier.Questline) == 14 then
		if player:getStorageValue(TheNewFrontier.Mission05.Angus) == 1 then
			...
			npcHandler:say("You bring up this topic again? I don't think our talks make any sense unless you prove that you are a supporter of science. Do you have some proof for your dedication to science?", cid)
			npcHandler.topic[playerId] = 35
		end
	elseif msgcontains(msg, "bluff") and player:getStorageValue(TheNewFrontier.Mission05.AngusKeyword) == 1 and player:getStorageValue(TheNewFrontier.Questline) == 14 and player:getStorageValue(TheNewFrontier.Mission05.Angus) == 1 then
	...
	elseif msgcontains(msg, "impress") and player:getStorageValue(TheNewFrontier.Mission05.AngusKeyword) == 2 and player:getStorageValue(TheNewFrontier.Questline) == 14 and player:getStorageValue(TheNewFrontier.Mission05.Angus) == 1 then
		if npcHandler.topic[playerId] == 1 then
			if player:getStorageValue(TheNewFrontier.Mission05.Angus) == 1 then
				npcHandler:say("A whole new land is indeed very tempting. I can hardly imagine all the wonders that await us there. I doubt I could stop our explorers from going there even if I tried to.", cid)
				player:setStorageValue(TheNewFrontier.Mission05.Angus, 3)
			end
		elseif npcHandler.topic[playerId] == 2 then
			if player:getStorageValue(TheNewFrontier.Mission05.Angus) == 1 then
				npcHandler:say({ "For all I've heard, this is rather a barren isle than a whole new land. There are so many places to explore in this world, I won't send our best explorers on account of a rumour." }, cid)
				player:setStorageValue(TheNewFrontier.Mission05.Angus, 3)
			end
		end
	...
	elseif msgcontains(msg, "mission") then
		...
		elseif player:getStorageValue(Storage.Quest.U7_6.ExplorerSociety.TheEctoplasm) == 47 and player:getStorageValue(Storage.Quest.U7_6.ExplorerSociety.QuestLine) == 47 then
			npcHandler:say({ "The research on ectoplasm makes good progress. Now we need some spectral article. Our scientists think a spectral dress would be a perfect object for their studies ...", "The bad news is that the only source to got such a dress is the queen of the banshees. Do you dare to seek her out?" }, cid)
		...
		elseif player:getStorageValue(Storage.Quest.U7_6.ExplorerSociety.TheSpectralDress) == 49 and player:getStorageValue(Storage.Quest.U7_6.ExplorerSociety.QuestLine) == 48 then
			npcHandler:say("Have you acquired the spectral dress we need?", cid)
		...
		elseif player:getStorageValue(Storage.Quest.U7_6.ExplorerSociety.TheSpectralDress) == 50 and player:getStorageValue(Storage.Quest.U7_6.ExplorerSociety.QuestLine) == 50 then
			npcHandler:say({ "With the objects you've provided our researchers will make steady progress. Still we are missing some test results from fellow explorers ...", "Please travel to our base in Northport and ask them to mail us their latest research reports. Then return here and ask about new missions." }, cid)
	...
	elseif msgcontains(msg, "butterfly hunt") then
		...
		elseif player:getStorageValue(Storage.Quest.U7_6.ExplorerSociety.TheButterflyHunt) == 11 and player:getStorageValue(Storage.Quest.U7_6.ExplorerSociety.QuestLine) == 11 then
			npcHandler:say({ "This preparation kit will allow you to collect a blue butterfly you have killed ...", "Just use it on the fresh corpse of a blue butterfly, return the prepared butterfly to me and give me a report of your butterfly hunt." }, cid)
			npcHandler.topic[playerId] = 0
		...
		elseif player:getStorageValue(Storage.Quest.U7_6.ExplorerSociety.TheButterflyHunt) == 14 and player:getStorageValue(Storage.Quest.U7_6.ExplorerSociety.QuestLine) == 14 then
			npcHandler:say({ "This preparation kit will allow you to collect a red butterfly you have killed ...", "Just use it on the fresh corpse of a red butterfly, return the prepared butterfly to me and give me a report of your butterfly hunt." }, cid)
			npcHandler.topic[playerId] = 0
			player:addItem(4863, 1)
	...
	elseif msgcontains(msg, "elven book") then
	...
	elseif msgcontains(msg, "yes") then
		...
		elseif npcHandler.topic[playerId] == 3 then
			if player:removeItem(4845, 1) then
				...
				npcHandler:say({ "Excellent, you brought just the tool we need! Of course it was only a simple task. However ...", "I officially welcome you to the explorer society. From now on you can ask for missions to improve your rank." }, cid)
				npcHandler.topic[playerId] = 0
			end
		elseif npcHandler.topic[playerId] == 4 then
			...
			npcHandler:say({ "So listen please: Take this ice pick and use it on a block of ice in the caves beneath Folda. Get some ice and bring it here as fast as you can ...", "Should the ice melt away, report on your ice delivery mission anyway. I will then tell you if the time is right to start another mission." }, cid)
			npcHandler.topic[playerId] = 0
		...
		elseif npcHandler.topic[playerId] == 7 then
			...
			npcHandler:say({ "This preparation kit will allow you to collect a purple butterfly you have killed ...", "Just use it on the fresh corpse of a purple butterfly, return the prepared butterfly to me and give me a report of your butterfly hunt." }, cid)
			npcHandler.topic[playerId] = 0
			player:addItem(4863, 1)
		...
		elseif npcHandler.topic[playerId] == 15 then
			...
			npcHandler:say({ "You have indeed the spirit of an adventurer! In the south-east of Tiquanda is a small settlement of the lizard people ...", "Beneath the newly constructed temple there, the lizards hide the said urn. Our attempts to acquire this item were without success ...", "Perhaps you are more successful." }, cid)
			npcHandler.topic[playerId] = 0
		...
		elseif npcHandler.topic[playerId] == 17 then
			...
			npcHandler:say({ "Excellent! So travel to the city of Darashia and then head north-east for the pyramid ...", "If any documents are left, you probably find them in the catacombs beneath. Good luck!" }, cid)
			npcHandler.topic[playerId] = 0
		...
		elseif npcHandler.topic[playerId] == 19 then
			...
			npcHandler:say({ "You are a brave soul. As far as we can tell, the orcs maintain some sort of training facility in some hill in the north-east of their city ...", "There you should find lots of their war wolves and hopefully also some of the orcish powder. Good luck!" }, cid)
			npcHandler.topic[playerId] = 0
		...
		elseif npcHandler.topic[playerId] == 31 then
			if player:removeItem(4836, 1) then
				...
				npcHandler:say("Just in time! With this spectral article we can start the final phase of our research.", cid)
				npcHandler.topic[playerId] = 0
			end
			-- Spectral Dress
			-- Spectral Stone
		elseif npcHandler.topic[playerId] == 32 then
			npcHandler:say("Good, just take this spectral essence and use it on the strange carving in this building as well as on the corresponding tile in our base at Northport ...", "As soon as you have charged the portal tiles that way, report about the spectral portals.", cid)
			npcHandler.topic[playerId] = 0
		...
		elseif npcHandler.topic[playerId] == 34 then
			...
			-- The New Frontier
		elseif npcHandler.topic[playerId] == 35 then
			if player:getStorageValue(TheNewFrontier.Questline) == 14 and player:getStorageValue(TheNewFrontier.Mission05.Angus) == 2 and player:removeItem(10011, 1) then
				npcHandler:say("What a strange map. I wonder if any of our explorers recognises this coastal lines. However, you earned yourself another chance to convince me. Why do you think that Farmine would be interesting for us?", cid)
				player:setStorageValue(TheNewFrontier.Mission05.Angus, 1)
				npcHandler.topic[playerId] = 2
			end
			-- Explorer Brooch
		elseif npcHandler.topic[playerId] == 36 then
	...
	elseif msgcontains(msg, "no") then
		...
		-- Answer No
		-- Skull Of Ratha / Giant Smithhammer
	elseif msgcontains(msg, "skull of ratha") and player:getStorageValue(Storage.Quest.U7_6.ExplorerSociety.SkullOfRatha.Bag1) == 1 then
		npcHandler:say({
	...
	elseif msgcontains(msg, "giant smith hammer") and player:getStorageValue(Storage.Quest.U7_6.ExplorerSociety.GiantSmithHammer.Hammer) == 1 then
		...
		-- Explorer Brooch
	elseif msgcontains(msg, "brooch") and player:getStorageValue(Storage.Quest.U7_6.ExplorerSociety.ExplorerBrooch) == 1 then
		npcHandler:say({
