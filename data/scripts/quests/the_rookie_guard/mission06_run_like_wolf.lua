...
local warWolfDenHole = ItemEvent()

function warWolfDenHole.onStepOn(creature, item, position, fromPosition)
	local player = creature:getPlayer()
	if not player then
		return true
	end
...
local warWolfDenTiles = ItemEvent()

function warWolfDenTiles.onStepOn(creature, item, position, fromPosition)
	local player = creature:getPlayer()
	if not player then
		return true
	end
...
local warWolfDenBoostTiles = ItemEvent()

function warWolfDenBoostTiles.onStepOn(creature, item, position, fromPosition)
	local player = creature:getPlayer()
	if not player then
		return true
	end
...
local poacherCorpse = ItemEvent()

poacherCorpse:type("use")
...
local skinningKnife = ItemEvent()

skinningKnife:type("use")
...
local warWolfDenChest = ItemEvent()

warWolfDenChest:type("use")
