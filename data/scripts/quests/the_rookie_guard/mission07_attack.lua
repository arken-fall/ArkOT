...
local libraryVaultSteps = ItemEvent()

function libraryVaultSteps.onStepOn(creature, item, position, fromPosition)
	local player = creature:getPlayer()
	if not player then
		return true
	end
...
local fireFields = ItemEvent()

function fireFields.onStepOn(creature, item, position, fromPosition)
	local player = creature:getPlayer()
	if not player then
		return true
	end
...
local destroyFieldRune = ItemEvent()

destroyFieldRune:type("use")
