...
-- Lions rock entrance
local lionsRockEntrance = ItemEvent()

function lionsRockEntrance.onStepOn(creature, item, position, fromPosition)
...
local lionsRockTranslationScroll = ItemEvent()

function lionsRockTranslationScroll.onStepOn(creature, item, position, fromPosition)
	local player = creature:getPlayer()
	if not player then
		return true
	end
...
-- Lions rock sign
local lionsRockSigns = ItemEvent()

function lionsRockSigns.onStepOn(creature, item, position, fromPosition)
	local player = creature:getPlayer()
	if not player then
		return
	end
...
-- Lions rock message
local lionsRockMessage = ItemEvent()

function lionsRockMessage.onStepOn(creature, item, position, fromPosition)
	local player = creature:getPlayer()
	if not player then
		return
	end
