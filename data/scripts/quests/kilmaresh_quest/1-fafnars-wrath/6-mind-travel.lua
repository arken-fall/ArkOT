...
local leftMirror = ItemEvent()

function leftMirror.onStepOn(creature)
...
local rightMirror = ItemEvent()

function rightMirror.onStepOn(creature)
	if creature:isPlayer() then
...
local memoryShards = ItemEvent()

memoryShards:type("use")
...
local energyField = ItemEvent()

function energyField.onStepOn(creature, item, position, fromPosition)
