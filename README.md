...
- **Prey and bestiary (Phase D, honest empties)** — the prey widget gets three locked slots and prices at login and on request, the bestiary gets empty race, overview and charm lists, so the windows open cleanly. The systems behind them are not written yet; the packets are shaped exactly as the client reads them so that when they are, only the data changes.
...
Still to come: the systems behind prey and bestiary (slots, race tracking, charms), the exaltation forge, the wheel of destiny and the 12.x store protocol, which are stubbed off so the client is in-world without them; and a real-map datapack in BlackTek's Lua/TOML layout.
