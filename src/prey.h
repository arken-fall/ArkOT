...
namespace BlackTek::Prey
{
	...
	struct Slot
	{
		...
		[[nodiscard]] bool isOccupied() const noexcept		{ return state == State::Active and selected_race != 0; }
		[[nodiscard]] bool canSelect() const noexcept		{ return state == State::Selection or state == State::SelectionChangeMonster or state == State::ListSelection or state == State::WildcardSelection; }
	...
	class System
	{
		public:
			...
			void action(const PlayerPtr& player, uint8_t slotId, Action action, uint8_t index, uint16_t raceId, Slot::Option option) const;
