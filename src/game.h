...
class Game
{
	public:
		...
		void playerStoreBrowse(uint32_t playerId, uint8_t action, const std::string& text, uint8_t subAction, uint32_t offerId);
		void playerRequestStoreHistory(uint32_t playerId, uint32_t page, uint8_t perPage);
		void playerStoreOfferDescription(uint32_t playerId, uint32_t offerId);
		void playerTransferCoins(uint32_t playerId, const std::string& recipientName, uint32_t amount);
		...
		void addMagicEffect(const Position& pos, const uint16_t effect, std::span<const CreaturePtr> pre_cache);
