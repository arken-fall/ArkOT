...
{
	// ids run across every file, and files load in name order so a quest keeps its id from one boot to the next
	auto isQuestFile = [](const std::filesystem::directory_entry& entry) { return entry.is_regular_file() and entry.path().extension() == ".toml"; };
	std::vector<std::filesystem::path> files;
	for (const auto& entry : std::filesystem::recursive_directory_iterator("data/quests/") | std::views::filter(isQuestFile))
	...
	for (const auto& path : files)
	{
		try {
			...
			BlackTek::Console::Error("Quests::loadFromToml: failed to parse {}: {}", path.string(), err.description());
			return false;
		}
	}
	return true;
